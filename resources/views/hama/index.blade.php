        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $(function() {
                ...
                if (dt_basic_table.length) {
                    ...
                    $('div.head-label').html('<h5 class="card-title mb-0">{{ __('Data Hama') }}</h5>');
                }

                // Add New record
                // On form submit, if form is valid
                fv.on('core.form.valid', function() {
                    var hama_kode = $('.add-new-record .dt-kode-hama').val(),
                        hama_nama = $('.add-new-record .dt-nama-hama').val(),
                        metode_id = $('.add-new-record .dt-metode-id').val();

                    if (hama_kode != '') {
                        $.ajax({
                            url: '{{ route('master.hama.store') }}',
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                hama_kode: hama_kode,
                            ...
                            success: function(response) {
                                ...
                                if (response.status == 'success') {
                                    ...
                                    $('.add-new-record .dt-kode-hama').val('');
                                    $('.add-new-record .dt-nama-hama').val('');
                                    $('.add-new-record .dt-metode-id').val('');
                                    offCanvasEl.hide();
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Success',
                ...
                fv = FormValidation.formValidation(formAddNewRecord, {
                    fields: {
                        hama_kode: {
                            validators: {
                                notEmpty: {
                                    message: '{{ __('Kode Hama perlu diisi') }}'
                                }
                            }
                        },
                        hama_nama: {
                            validators: {
                                notEmpty: {
                                    message: '{{ __('Nama Hama perlu diisi') }}'
                                }
                            }
                        },
                        metode_id: {
            ...
            $('#datatables-basic').on('draw.dt', function() {
                        ...
                        editRecord.forEach(el => {
                            el.addEventListener('click', function() {
                                ...
                                offCanvasUpdateElement.querySelector(
                                    '.dt-id-edit-hama').value = data.id;
                                offCanvasUpdateElement.querySelector(
                                    '.dt-kode-hama').value = data.hama_kode;
                                offCanvasUpdateElement.querySelector(
                                    '.dt-nama-hama').value = data.hama_nama;
                                offCanvasUpdateElement.querySelector(
                                    '.dt-metode-id').value = data.metode_id;
                ...
                fvedit.on('core.form.valid', function() {
                    var id = $('.update-record .dt-id-edit-hama').val(),
                        hama_kode = $('.update-record .dt-kode-hama').val(),
                        hama_nama = $('.update-record .dt-nama-hama').val(),
                        metode_id = $('.update-record .dt-metode-id').val();

                    if (hama_kode != '') {
                        $.ajax({
                            url: '{{ route('master.hama.update', ['id' => ':id']) }}'
                                .replace(
                                    ':id', id),
                            type: 'PATCH',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                if (response.status == 'success') {
                                    // Handle success
                                    $('#datatables-basic').DataTable().ajax.reload();
                                    $('.update-record .dt-id-edit-hama').val('');
                                    $('.update-record .dt-kode-hama').val('');
                                    $('.update-record .dt-nama-hama').val('');
                                    $('.update-record .dt-metode-id').val('');
                        ...
                        element.addEventListener('click', function() {
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        url: '{{ route('master.hama.destroy', ['id' => ':id']) }}'
                                            .replace(':id', data
                                                .id),
                                        type: "DELETE",
                                        data: {
...
@section('content')
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master /</span> Hama</h4>
            ...
            <table class="table border-top" id="datatables-basic">
                <thead>
                    <tr>
                        ...
                        <th>Kode</th>
                        <th>Nama</th>
                        <th style="display: none">ID_Metode</th>
                        <th>Metode</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
    ...
    <div class="offcanvas offcanvas-end" id="add-new-record">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="add-new-record pt-0 row g-2" id="form-add-new-record">
                <div class="col-sm-12">
                    <label class="form-label" for="hama_kode">{{ __('Kode Hama') }}</label>
                    <div class="input-group input-group-merge">
                        <input type="text" id="hama_kode" class="form-control dt-kode-hama" name="hama_kode"
                            placeholder="XX" aria-label="XX" required />
                    </div>
                </div>
                <div class="col-sm-12">
                    <label class="form-label" for="hama_nama">{{ __('Nama Hama') }}</label>
                    <div class="input-group input-group-merge">
                        <input type="text" id="hama_nama" name="hama_nama" class="form-control dt-nama-hama"
                            placeholder="Tikus / Lebah / dll" aria-label="Tikus / Lebah / dll" required />
                    </div>
                </div>
                <div class="col-sm-12">
                    <label class="form-label" for="metode_id">{{ __('Jenis Metode') }}</label>
                    <div class="input-group input-group-merge">
                        <select class="form-select dt-metode-id" id="metode_id" name="metode_id" aria-label="Pilih Metode"
                            required>
                            <option value="">Pilih Metode</option>
                            @foreach ($metode as $item)
                                <option value="{{ $item->id }}">{{ $item->metode_nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <div class="offcanvas offcanvas-end" id="update-record" tabindex="-1">
        ...
        <div class="offcanvas-body flex-grow-1">
            <form class="update-record pt-0 row g-2" id="form-update-record">
                <div class="col-sm-12">
                    <input type="hidden" id="id-edit" class="form-control dt-id-edit-hama" name="id-edit" required
                        readonly />
                </div>
                <div class="col-sm-12">
                    <label class="form-label" for="hama_kode">{{ __('Kode Hama') }}</label>
                ...
                <div class="col-sm-12">
                    <label class="form-label" for="metode_id">{{ __('Jenis Metode') }}</label>
                    <div class="input-group input-group-merge">
                        <select class="form-select dt-metode-id" id="metode_id" name="metode_id"
                            aria-label="Pilih Metode" required>
                            <option value="">Pilih Metode</option>
                            @foreach ($metode as $item)
                                <option value="{{ $item->id }}">{{ $item->metode_nama }}</option>
                            @endforeach
